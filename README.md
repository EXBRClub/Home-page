...
Quando existe uma sessão ativa, o botão Login da Home muda para Perfil. No perfil, o dono pode revelar o botão de edição clicando no avatar; o controle volta a ficar oculto após 30 segundos sem atividade. O membro pode escrever uma apresentação de até 220 caracteres, escolher sua classe e facção favoritas e marcar com uma estrela até cinco medalhas para exibir na Comunidade. A apresentação aparece em um monitor tático com falhas discretas; classe e facção ocupam marcadores próprios sobre o cartão do avatar. A escolha de facção utiliza os símbolos visuais de New Conglomerate, Terran Republic, Vanu Sovereignty e Nanite Systems Operatives hospedados permanentemente no Cloudinary; o marcador do cartão e a lista comunitária usam a mesma imagem escolhida. Administradores recebem acesso ao painel protegido `pages/admin.html`, com abas próprias para gestão de soldados, edição do catálogo de medalhas e galeria. A concessão permite escolher livremente a data da conquista, inclusive de forma retroativa. A mesma medalha pode ser concedida mais de uma vez em datas ou operações diferentes; a remoção administrativa fica no perfil consultado. Medalhas sem imagem usam `assets/icons/dock/recrutamento.png`. Imagens externas de medalhas são copiadas para o Cloudinary e entregues em PNG permanente; falhas de exibição ainda retornam ao ícone padrão. Perfis, comunidade e operações priorizam a imagem atual do catálogo, inclusive para concessões antigas. As medalhas são organizadas como um inventário de três colunas, com nove posições iniciais e expansão vertical. Slots ganhos mostram o ícone e o estado de favorito; espaços ainda não ocupados permanecem vazios. Ao selecionar uma medalha, o membro vê sua imagem ampliada, nome, descrição, operação, data e, no próprio perfil, o comando de favoritar. Em computadores com mouse, a imagem ampliada possui inspeção por zoom de 2,4× que acompanha o cursor; celulares mantêm a visualização estática. Quando um administrador consulta um perfil, o botão `+` do cabeçalho abre um catálogo de concessão em nove colunas, com pesquisa, operação e data retroativa.
